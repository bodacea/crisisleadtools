...
class SkypechatConnector:
    ...
    def make_buddies(self, userids, message):
        ...
        return(newbuddies, oldbuddies)
    
    
    #Create a chatroom
    #UserIds is a comma-separated list of skypeids, e.g. "user1id,user2id,user3id"
    def create_chatroom(self, chatname, userids, invitemessage):

        #Send connect requests to everyone you're not buddies with
        newbuddies, oldbuddies = self.make_buddies(
            userids,
            "Please accept my request so I can add you to the " +
            chatname + " Skypegroup")
        
        #Create chat group in Skype, with the given list of users
        #NB USERS in this case means the handles to the users - i.e. a skype-generated
        #list. 
        firstuser = userids.split(",")[0]
        messid = self.sconn.CreateChatWith(userids).SendMessage(invitemessage)
        groupid = messid.Chat
        groupid._SetTopic(chatname)
        
        ##self.sconn.chats[groupid].addMembers(userids)
        #group.FriendlyName = chatname
        #group.Description = "This is the group's description. Please edit it"
        #groupid.SendMessage(invitemessage)
        print("Creating skype group " + chatname)
##        for userid in userids:
##            print("Trying to add " + userid)
##            if userid not in newbuddies:
##                print("I know this person!")
##                group.AddUser(userid)

        ##groupid = group.GetCustomGroupId()
        groupid = 0

        ##Notes on groups
        #gid = self.sconn.CreateGroup("This is a test Skype group")
        #gid.AddUser("userid1")
        #gid.AddUser("userid2")
        #users = gid.Users
        #gid._SetDisplayName("this is a test name")
        #dname = gid.DisplayName
        #Options... create group by hand, then grab it by name and add names to it.
