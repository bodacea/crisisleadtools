...
'''
Load list of Ningsite members from CSV file produced by Ningsite
Assumes that data is an excel file, with member data in sheet "memberdata"
Filetype is either "ningmembers" or "ninggroups"

Sara-Jayne Farmer
2012
'''
def import_ning_members(filename, filetype="ningmembers"):

    ningmembers = {}
    
    if filetype == "ningmembers":

        #This is the excel spreadsheet of member details archived by Ning
        wbk = xlrd.open_workbook(filename)
        sh = wbk.sheet_by_name("memberdata")

        for i in range(1,sh.nrows):
            ningaddress = sh.cell_value(i,1)
            slash  = ningaddress.rfind("/")
            ningid = ningaddress[slash+3:]

            ningmembers[ningid] = {}
            ningmembers[ningid]['fullname']       = sh.cell_value(i,0)
            ningmembers[ningid]['ningemail']      = sh.cell_value(i,2)
            #3 is gender
            ningmembers[ningid]['ninglocation']   = sh.cell_value(i,4)
            ningmembers[ningid]['ningcountry']    = sh.cell_value(i,5)
            #6 is zip; 7 is age; 8 is birthdate
            ningmembers[ningid]['ningdatejoined'] = sh.cell_value(i,9)
            #10 is receiving broadcasts; 11 is receiving emails
            ningmembers[ningid]['ninglastvisit']  = sh.cell_value(i,12)
            ningmembers[ningid]['organisation']   = sh.cell_value(i,13)
            ningmembers[ningid]['website']        = sh.cell_value(i,14)
            ningmembers[ningid]['email']          = sh.cell_value(i,15)

            skypeid = sh.cell_value(i,16)
            if type(skypeid) == str or type(skypeid) == unicode:
                skypeid = skypeid.lower()
            ningmembers[ningid]['skypeid']        = skypeid

            ningmembers[ningid]['twitterid']      = sh.cell_value(i,17)
            ningmembers[ningid]['bio']            = sh.cell_value(i,18)
            #19 is Ushahidi experience; 20 is Ushahidi tasks; 21 is team to join
            ningmembers[ningid]['skills']         = sh.cell_value(i,22)
            ningmembers[ningid]['languages']      = sh.cell_value(i,23)
            ningmembers[ningid]['location']       = sh.cell_value(i,24)

    else:
        
        #This is a spreadsheet created from the json files archived by Ning
        wbk = xlrd.open_workbook(filename)
        sh = wbk.sheet_by_name("MemberSummary")
        for i in range(1,sh.nrows):
            ningid = sh.cell_value(i,4)
            skypeid = sh.cell_value(i,18)
            fullname = sh.cell_value(i,18)
            ningmembers[ningid] = {}
            ningmembers[ningid]['fullname'] = fullname
            ningmembers[ningid]['skypeid']  = skypeid
