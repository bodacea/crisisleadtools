...
def read_skypemessages_cutpaste(infile="", outfile="", verbose="N"):
    ...
    skypemessages = read_skypemessages_cutpaste_stream(fin, outfile, verbose)
    
    return(skypemessages)


'''
Read Skype messages from a cut and paste file presented as a datastream
Needed for interaction with Django.
'''
def read_skypemessages_cutpaste_stream(fin, outfile="", verbose="N"):
    ...
    for line in fin:
        ...
        if len(timedata) == 0:
            ...
            outrow = [m_from, line, m_timestamp, "EMOTED"]
        ...
        #Deal with Skype action messages
        if mdata[:3] == "***" and mdata[-3:] == "***":
            ...
            if cols != []:
                ...
                if cols != []:
                    ...
                    #A different type of action message... file under ACTION
                    m_from = ""
                    m_type = "ACTION"
                    m_body = mdata[4:-4]
        else:
...
'''
Read in list of Skypeids from a csv file
with no header and ids in the first column

Sara-Jayne Farmer
2012
'''
def import_skypeids(csvfile):
    #Import skype lists
    skypeids = []
    skypenames = {}
    fskype = open(csvfile, 'rb')
...
Assumes that skypemessages is an array with 1 row per message, where messages
are arranged as [from, body, timestamp, type]
...
def count_countributions(skypemessages):
    ...
    for m in skypemessages:
        ...
        messtype = m[3]
        if messtype in ['SAID']:
            cnt.setdefault(fromname, {})
            cnt[fromname].setdefault('num_messages', 0)
            cnt[fromname]['num_messages'] += 1
        #cnt[m.FromDisplayName] += 1 #Used for raw skype feed
...
def graph_users(users, userstats):
    addgraph = nx.DiGraph()
...
'''
def word_histogram(chatstring):
    print("Most common words used: ")
    texthist = nltk.FreqDist(chatstring)
    histdict = {}
    for item in texthist.iteritems():
        histdict[item[0]] = item[1]
##    for token in sorted(texthist.keys()[:20]):
##        print(token + " " + str(texthist[token]) + "times")
    return texthist, histdict


'''
Analyse Skypechat

Assumes chat has been read into an array, with one item per
skype message or action, and each item in the form
[m_from, m_body, m_timestamp, m_type]
...
def analyse_skypechat(skypemessages, skypeusers):
    ...
    stopwords = nltkstops + ["think", "know", "like", "need", "would", "i'm",
                             "get", "going", "one", "also", "don't", "go",
                             "might", "i've", "yes", "i'll", "it's", "could",
                             "we're"]
    chatwords = []
    userstats = {}
    allwords = r"\w+(?:[-']\w+)*|'|[-.(]+|\S\w*"
    for row in skypemessages:
        ...
        userstats.setdefault(person, {})
        userstats[person].setdefault('num_messages', 0)
        ...
        if action not in ["ADDEDMEMBERS", "HASLEFT"]:
            ...
            chatwords = chatwords + list(set(foundwords).difference(stopwords))
            userstats[person]["num_messages"] += 1
        else:
    ...
    addgraph = graph_users(skypeusers, userstats)
    ##chatstring = " ".join(chatwords)
    texthist, histdict = word_histogram(chatwords)
    
    return userstats, addgraph, histdict
